...
int main(int argc, char *argv[]){
    ...
    ifm_load_calls += PW_H_in * PW_W_in * PW_C_IN / BRAM_WIDTH_IN_BYTE;
    printf("[LOGS] IFM BRAM Loaded.\n");
    
    // ============== Load PW weight ================
    printf("[LOGS] Loading PW Weight BRAMs...\n");
    int rows_per_pw_filter = PW_C_IN / BRAM_WIDTH_IN_BYTE;
    for (int tile = 0; tile < PW_NUM_OF_FILTER / NUM_OF_PE; tile++) {
        for (int bram_indx = 0; bram_indx < NUM_OF_BRAM; bram_indx++) {
            ...
            pw1_load_calls += rows_per_pw_filter;
        }
    }
    ...
    int se_pw1_rows_per_filter = (SE_PW_1_CIN + BRAM_WIDTH_IN_BYTE - 1) / BRAM_WIDTH_IN_BYTE;
    for (int tile = 0; tile < SE_PW_1_COUT / NUM_OF_SE_PE; tile++) {
        for (int bram_indx = 0; bram_indx < NUM_OF_SE_BRAM; bram_indx++) {
            for (int row_indx = 0; row_indx < se_pw1_rows_per_filter; row_indx++) {
            ...
            se1_load_calls += se_pw1_rows_per_filter;
        }
    }
    ...
    int se_pw2_rows_per_filter = (SE_PW_2_CIN + BRAM_WIDTH_IN_BYTE - 1) / BRAM_WIDTH_IN_BYTE;
    for (int tile = 0; tile < SE_PW_2_COUT / NUM_OF_SE_PE; tile++) {
        for (int bram_indx = 0; bram_indx < NUM_OF_SE_BRAM; bram_indx++) {
            for (int row_indx = 0; row_indx < se_pw2_rows_per_filter; row_indx++) {
                int filter_idx = tile * NUM_OF_SE_PE + bram_indx;
            ...
            se2_load_calls += se_pw2_rows_per_filter;
        }
    }
    ...
    int pw_last_rows_per_filter = PW_LAST_CIN / BRAM_WIDTH_IN_BYTE;
    for (int tile = 0; tile < PW_LAST_COUT / NUM_OF_PE; tile++) {
        for (int bram_indx = 0; bram_indx < NUM_OF_BRAM; bram_indx++) {
            for (int i = 0; i < pw_last_rows_per_filter; i++) {
                int filter_idx = tile * NUM_OF_PE + bram_indx;
            ...
            pw_last_load_calls += pw_last_rows_per_filter;
        }
    }
    printf("[LOGS] PW Weight BRAMs Loaded.\n");
    ...
    for(int tile = 0; tile < PW_NUM_OF_FILTER / NUM_OF_PE; tile++){
        for(int ho = 0; ho < PW_H_out; ho++){
            for(int wo = 0; wo < PW_W_out; wo++){
                ...
                int ifm_row_indx = (ho * PW_W_in + wo) * rows_per_pw_filter;
    ...
    for(int chuck_ofm = 0; chuck_ofm < SE_PW_1_COUT / NUM_OF_SE_PE; chuck_ofm++){
        ...
        for(int row_ifm = 0; row_ifm < se_pw1_rows_per_filter; row_ifm++){
            cycles_se1++;
            int w_row = chuck_ofm * se_pw1_rows_per_filter + row_ifm;
            pw_pe_compute(&se_pw_pe_1_arr[0], GAP_BRAM, row_ifm, SE_PW_1_W1_BRAM, w_row);
            pw_pe_compute(&se_pw_pe_1_arr[1], GAP_BRAM, row_ifm, SE_PW_1_W2_BRAM, w_row);
            pw_pe_compute(&se_pw_pe_1_arr[2], GAP_BRAM, row_ifm, SE_PW_1_W3_BRAM, w_row);
            pw_pe_compute(&se_pw_pe_1_arr[3], GAP_BRAM, row_ifm, SE_PW_1_W4_BRAM, w_row);
        }
    ...
    for(int row_ofm = 0; row_ofm < SE_PW_2_COUT / NUM_OF_SE_PE; row_ofm++){
        ...
        for(int row_ifm = 0; row_ifm < se_pw2_rows_per_filter; row_ifm++){
            cycles_se2++;
            int8_t *ifm_row = SE_PW_1_ACC_BRAM[row_ifm];
            int w_row = row_ofm * se_pw2_rows_per_filter + row_ifm;
            pw_pe_compute(&se_pw_pe_2_arr[0], (int8_t (*)[16])ifm_row, 0, SE_PW_2_W1_BRAM, w_row);
            pw_pe_compute(&se_pw_pe_2_arr[1], (int8_t (*)[16])ifm_row, 0, SE_PW_2_W2_BRAM, w_row);
            pw_pe_compute(&se_pw_pe_2_arr[2], (int8_t (*)[16])ifm_row, 0, SE_PW_2_W3_BRAM, w_row);
            pw_pe_compute(&se_pw_pe_2_arr[3], (int8_t (*)[16])ifm_row, 0, SE_PW_2_W4_BRAM, w_row);
        }
    ...
    for(int tile = 0; tile < PW_LAST_COUT / NUM_OF_PE; tile++){
        for(int ho = 0; ho < PW_LAST_H; ho++){
            for(int wo = 0; wo < PW_LAST_W; wo++){
                ...
                int ifm_row_indx = (ho * PW_LAST_W + wo) * pw_last_rows_per_filter;
                int w_row_indx = tile * pw_last_rows_per_filter;
                for(int i = 0; i < pw_last_rows_per_filter; i++){
                    cycles_pw_last++;
