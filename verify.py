            ...
            if 'BRAM' in line:
                try:
                    ...
                    if '/' in parts[1]:
                        ...
                        usage[f"{key} Ping"] = float(vals[0].strip())
                        usage[f"{key} Pong"] = float(vals[1].strip())
            
            # Cycle parsing
            elif any(x in line for x in ['Cycles', 'Calls', 'calls', 'COMPUTE', 'EXECUTION']):
                try:
...
if __name__ == "__main__":
    ...
    if all_success:
        ...
        for k in sorted(global_usage.keys(), key=sort_priority):
            unit = "calls" if any(x in k.upper() for x in ["CYCLES", "CALLS", "COMPUTE", "EXECUTION"]) else "KB"
            v = global_usage[k]
